 ...
 * (pelo código) decide o papel: resultado = receita − deduções − despesas.
 */
export function summarizeDre(
  rows: ExtractedRow[],
  meta: Pick<ExtractMeta, 'declaredResult'>
): DreSummary {
  let receita = 0
  let deducoes = 0
  let despesas = 0
  ...
  for (const r of rows) {
    ...
    const kind = classifyKindByCode(r.account_code, r.account_name)
